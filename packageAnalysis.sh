...
docker save ghcr.io/thenrique82/demo-database:$BUILD_VERSION -o demo-database.tar
...
if [ $status -eq 0 ]; then
	docker save ghcr.io/thenrique82/demo-backend:$BUILD_VERSION -o demo-backend.tar
	...
	if [ $status -eq 0 ]; then
		docker save ghcr.io/thenrique82/demo-frontend:$BUILD_VERSION -o demo-frontend.tar
