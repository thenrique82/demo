...
if [ -z "$PROVISIONED" ]; then
  ...
  $KUBECTL_CMD --kubeconfig=./.kubeconfig set image deployment database database=ghcr.io/thenrique82/demo-database:$BUILD_VERSION
  $KUBECTL_CMD --kubeconfig=./.kubeconfig set image daemonset backend backend=ghcr.io/thenrique82/demo-backend:$BUILD_VERSION
  $KUBECTL_CMD --kubeconfig=./.kubeconfig set image daemonSet frontend frontend=ghcr.io/thenrique82/demo-frontend:$BUILD_VERSION
fi
